    ...
    if($cek->num_rows){
        ...
        if(strlen($password_baru) >= 5){
            ...
            if ($password_baru == $password_lama) {
                 header("location:ProfilUser.php?pesan=pwdtdksama");
                    ...
                    if($update){
                        //kondisi jika proses query UPDATE berhasil
                         header("location:ProfilUser.php?pesan=berhasilpwd");
                    }else{
                        //kondisi jika proses query gagal
                         header("location:ProfilUser.php?pesan=gagalpwd");
                    }                   
                }else{
                    //kondisi jika password baru beda dengan konfirmasi password
                     header("location:ProfilUser.php?pesan=gagalkonfirmasi");
                }
            }else{
                //kondisi jika password baru yang dimasukkan kurang dari 5 karakter
                 header("location:ProfilUser.php?pesan=min");
            }
        }else{
            //kondisi jika password lama tidak cocok dengan data yang ada di database
             header("location:ProfilUser.php?pesan=pwdlamatdkcocok");
        }
