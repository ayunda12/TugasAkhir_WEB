...
if (isset($_POST['id'])) {
    ...
    if ($foto_size > 2097152) {
       ...
       header("location:layanan.php?pesan=size");
       ...
       if ($foto_nama != "") {
          ...
          if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $get_foto = "SELECT gambar FROM layanan WHERE id='$id'";
            ...
            $query = mysqli_query($koneksi, "UPDATE layanan SET nama_layanan ='$nama_layanan',keterangan='$keterangan', gambar ='$foto_nama_new' WHERE id='$id'");
           

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	header("location:layanan.php?pesan=berhasiledit");
            } else {
                header("location:layanan.php?pesan=gagaledit");
            }

        } else { 
        	// Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
        	header("location:layanan.php?pesan=ekstensi");        }

        }else{
            $query = mysqli_query($koneksi, "UPDATE layanan SET nama_layanan ='$nama_layanan',keterangan='$keterangan' WHERE id='$id'");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
                header("location:layanan.php?pesan=berhasiledit");
            }else {
                header("location:layanan.php?pesan=kosong");
            }
        }
