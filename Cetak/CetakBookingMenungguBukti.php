...
$query = mysqli_query($koneksi,"SELECT * FROM booking JOIN user ON booking.id_user = user.id_user JOIN paket ON booking.id_paket = paket.id_paket WHERE status ='Menunggu Bukti Transfer' GROUP BY tgl_booking");
$row = mysqli_num_rows($query);
if($row>0){
$html = '<center><h2>Dyah Ayu Salon</h2><h3>Laporan Data Pemesanan Paket Pernikahan (Status Pemesanan : Menunggu Bukti Transfer)</h3></center><hr><br/>
';
...
$dompdf->stream('LaporanPesanan_TungguBukti.pdf',array("Attachment"=>0));
}else{
