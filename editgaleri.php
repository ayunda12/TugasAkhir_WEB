    ...
    if ($foto_size > 2097152) {
       ...
       if ($foto_nama != "") {
          ...
          if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $get_foto = "SELECT foto FROM galeri WHERE id='$id'";
            ...
            $query = mysqli_query($koneksi, "UPDATE galeri SET foto ='$foto_nama_new', keterangan='$keterangan' WHERE id='$id'");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	header("location:galeri.php?pesan=berhasiledit");
            } else {
                header("location:galeri.php?pesan=gagaledit");
            }

        } else { 
        	// Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
        	header("location:galeri.php?pesan=ekstensi");        }

        }else{
            $query = mysqli_query($koneksi, "UPDATE galeri SET keterangan='$keterangan' WHERE id='$id'");
